...
mod tests {
    ...
    async fn setup_search(partial_match: bool) -> FullTextSearch {
    ...
    async fn test_basic_indexing_and_search() {
        let search = setup_search(false).await;
        ...
        // 测试简单搜索
        let (tokens, results) = search.search("hello").await.unwrap();
        ...
        // 验证评分机制
        let (_, results) = search.search("world").await.unwrap();
        assert_eq!(results.len(), 2);
    ...
    async fn test_index_consistency() {
        ...
        // 验证旧内容不可搜索
        let (_, results) = search.search("initial").await.unwrap();
        assert_eq!(results.len(), 0, "旧内容不应该可被搜索到");

        // 验证新内容可搜索
        let (_, results) = search.search("updated").await.unwrap();
    ...
    async fn test_partial_match() {
        let partial_search = setup_search(true).await;
        ...
        // 测试部分匹配搜索
        let (_, results) = partial_search.search("Rust Python").await.unwrap();
        assert_eq!(results.len(), 2); // 应该匹配包含 "rust" 或 "python" 的文档

        // 测试完整词组匹配
        let (_, results) = partial_search.search("programming").await.unwrap();
    ...
    async fn test_edge_cases() {
        ...
        let (_, results) = search.search("rust").await.unwrap();
        assert_eq!(results.len(), 1);

        // HTML内容
        search.index(6, "<p>Hello World</p><div>Rust</div>").await.unwrap();
        let (_, results) = search.search("hello world rust").await.unwrap();
        assert_eq!(results.len(), 1);

        // 特殊字符
        search.index(7, "rust#programming$language@test").await.unwrap();
        let (_, results) = search.search("rust programming language test").await.unwrap();
        assert_eq!(results.len(), 1);

        // Unicode字符
        search.index(8, "rust😀programming🚀language").await.unwrap();
        let (_, results) = search.search("rust programming language😀").await.unwrap();
        assert_eq!(results.len(), 1);
    ...
    async fn test_cjk_support() {
        ...
        // 中文搜索
        let (_, results) = search.search("编程").await.unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].0, 1);

        // 混合语言文档
        search.index(3, "学习 rust 和 python programming").await.unwrap();
        let (_, results) = search.search("rust python").await.unwrap();
        assert_eq!(results.len(), 1);

        // 中文标点符号
        search.index(4, "rust（编程）语言，开发。教程！").await.unwrap();
        let (_, results) = search.search("编程 语言").await.unwrap();
        assert_eq!(results.len(), 2);
    ...
    async fn test_ranking_and_relevance() {
        ...
        // 准备具有不同特征的文档
        partial_search.index(1, "rust programming").await.unwrap();
        partial_search.index(2, "rust programming guide").await.unwrap();
        partial_search.index(3, "rust programming complete tutorial").await.unwrap();
        partial_search.index(4, "rust").await.unwrap();
        partial_search.index(5, "rust rust rust").await.unwrap();  // 测试词频影响

        let (_, results) = partial_search.search("rust programming").await.unwrap();
    ...
    async fn test_error_recovery() {
        ...
        // 测试在清理后搜索
        search.clear_all_indexes().await.unwrap();
        let (_, results) = search.search("test").await.unwrap();
    ...
    async fn test_max_results_limit() {
        ...
        let tokenizer = Arc::new(Jieba::new());
        let limited_search = FullTextSearch::new(
            Arc::new(rd),
            tokenizer,
            false,
            3,  // max_results = 3
            "test_limited:".to_string(),
        );
        ...
        // 验证结果数量限制
        let (_, results) = limited_search.search("test").await.unwrap();
